...
class DragElementsContainer extends React.Component {
    ...
    changePosition = (id, event) => {
        event.target.closest('.draggableItem').style.zIndex = id
    }
    ...
    renderElements(type, index) {
        switch(type) {
            ...
            case 'gif':
                return(<DragElement color={this.props.color} changeIndex={this.changeIndex} key={index} index={index} type={type} src={this.props.element.src} />)
            case 'video':
